...
Mandatory evidence with `FAIL` or `UNKNOWN` must result in `NO_GO`.
...

{
  ...
  "subject": {
    ...
    "artifact_reference": "example.registry/ledger-api:1.0.0"
  ...
  "provenance": {
    ...
    "collected_by": "rehearsalctl",

    "input_checksum": "sha256:example"
...
Health or readiness failure                  -> NO_GO

Duplicate transaction                       -> NO_GO

Ledger integrity failure                    -> NO_GO

Missing mandatory rehearsal evidence        -> NO_GO

Critical unresolved security finding        -> NO_GO

Unavailable optional enterprise adapter     -> SKIPPED or CONDITIONAL_GO

Quality gate failure when configured        -> NO_GO
